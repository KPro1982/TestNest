class kpNestedTesterStore<Class T>
{
	// the root should reflect your mod root folder. This is tytically your mod name
	static string Root = "$profile:\\TestNest\\";
	protected string m_currentfilename;
	ref T m_data;
	
	void kpNestedTesterStore(string filename, T data)
	{
		auto trace = CF_Trace_2("kpNestedTesterStore", "kpNestedTesterStore").Add(filename).Add(data);
		...
		if(GetGame().IsDedicatedServer())
		{
			...
			GetRPCManager().AddRPC( "TestNest", "_kpNestedTesterStore_ReceiveCommandOnServer", this, SingleplayerExecutionType.Both );
		}
		else
		{
			GetRPCManager().AddRPC( "TestNest", "_kpNestedTesterStore_ReceiveDataOnClient", this, SingleplayerExecutionType.Both );		
		}
		...
		if(GetGame().IsDedicatedServer())
		{
			if(!OpenFile(m_currentfilename)) // attempt to load it
			{
				...
				CF_Log.Info("[kpNestedTesterStore::Constructor] no file found, creating file");
				CF_Log.Info("[kpNestedTesterStore::Constructor] updating client with loaded data.");
			}
			else
			{
				CF_Log.Info("[kpNestedTesterStore::Constructor] file found and loaded");
			}
	...
	void ~kpNestedTesterStore() 
    {	
		auto trace = CF_Trace_0("kpNestedTesterStore", "~kpNestedTesterStore");
	}
	...
	void Init()
	{
		auto trace = CF_Trace_0("kpNestedTesterStore", "Init");
	...
	void SetData(T data)
	{
		auto trace = CF_Trace_1("kpNestedTesterStore", "SetData").Add(data);
		m_data = data;
	}
	
	T GetData()
	{
		auto trace = CF_Trace_0("kpNestedTesterStore", "GetData");
		return m_data;
	}
	...
	{
		auto trace = CF_Trace_3("kpNestedTesterStore", "Process").Add(savedata).Add(command).Add(filename);
		if(GetGame().IsDedicatedServer())
		{
	...
	{
		auto trace = CF_Trace_1("kpNestedTesterStore", "SaveFile").Add(filename);
	...
	{
		auto trace = CF_Trace_1("kpNestedTesterStore", "NewFile").Add(filename);
		if(GetGame().IsDedicatedServer())
		{			
			SaveFile(filename, newfileData );  
		}
	 ...
	 bool OpenFile(string filename){
		auto trace = CF_Trace_1("kpNestedTesterStore", "Open").Add(filename);
	...
	{
		auto trace = CF_Trace_3("kpNestedTesterStore", "SendCommandToServer").Add(sendData).Add(command).Add(commandparam);	
		GetRPCManager().SendRPC("TestNest", "_kpNestedTesterStore_ReceiveCommandOnServer", new Param3< T, string, string >( sendData, command, commandparam ), true, NULL);
	}
	
	void SendDataToClient(T sendData)
	{
	
		GetRPCManager().SendRPC("TestNest", "_kpNestedTesterStore_ReceiveDataOnClient", new Param1< T>( sendData ), true, NULL);
	}
	
	// runs on the server
	void UpdateServer(T savedata = null)
	{
		auto trace = CF_Trace_1("kpNestedTesterStore", "UpdateServer").Add(savedata);
		if(GetGame().IsDedicatedServer())
		{
			CopyData(savedata);
			Print("[kpNestedTesterStore::UpdateServer()] data: ");
	...
	void UpdateClient()
	{
		auto trace = CF_Trace_0("kpNestedTesterStore", "UpdateClient");
		if(GetGame().IsDedicatedServer())
		{	
			SendDataToClient( m_data );
	...
	void _kpNestedTesterStore_ReceiveCommandOnServer( CallType type, ParamsReadContext ctx, PlayerIdentity sender, Object target ) 
	{		
		if(GetGame().IsDedicatedServer())
		{
			auto trace = CF_Trace_4("kpNestedTesterStore", "_kpNestedTesterStore_ReceiveCommandOnServer").Add(type).Add(ctx).Add(sender).Add(target);
	...
	void _kpNestedTesterStore_ReceiveDataOnClient( CallType type, ParamsReadContext ctx, PlayerIdentity sender, Object target ) {
		if(!GetGame().IsDedicatedServer())
		{
			auto trace = CF_Trace_4("kpNestedTesterStore", "_kpNestedTesterStore_ReceiveDataOnClient").Add(type).Add(ctx).Add(sender).Add(target);;	
			Param1< ref T > data;
			if ( !ctx.Read( data ) ) return;
